import { Controller, Post, Body, UseGuards, Get, Param, UseInterceptors, Req, Injectable, Scope, Res, HttpStatus } from '@nestjs/common';
...
import { AzureADGuard } from './auth.ad.strategy';
...
export class AuthorizationController {
  ...
  async login(@Body() user: CreateUserDto, @Req() request) {
    ...
    const authenticatedUser = await this.authorizationService.validateUser(
      user.username,
      user.password,
    );
  ...
  async callback(@Req() req, @Res({ passthrough: true }) response) {
    const aure_code = req.body.code; // get id_token from azure ad
